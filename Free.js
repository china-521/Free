(function (window) {
    window.tools = window.free = {
        ...
        getStyle: function getStyle(obj, name) {
         ...
         */
        move: function move(obj, attr, target, speed, callback) {
         ...
         */
        addClass: function addClass(obj, cn) {
         ...
         * 删除一个元素中指定的class属性
         */
        removeClass: function removeClass(obj, cn) {
            // 创建一个正则表达式
         ...
         * 如果元素中没有该类，则添加
         */
        toggleClass: function toggleClass(obj, cn) {
            // 检查obj中是否有cn这个class
            if (hasClass(obj, cn)) {
                // 如果有cn，则删除
                removeClass(obj, cn);
        ...
        hasClass: function hasClass(obj, cn) {
            // 判断obj中有没有cn
         ...
         *  fun, 回调函数。--->function(){}
         */
        myClickId: function myClickId(obj, fun) {
            var btn = document.getElementById(obj);
            btn.onclick = fun;
        },
        myClickAll: function myClickAll(obj, fun) {
         ...
         *      obj：被拖拽对象
         */
        limitDrag: function limitDrag(obj) {
            // 为obj绑定鼠标按下事件
            // 当鼠标在被拖拽元素上按下时，开始拖拽 onmousedown
            obj.onmousedown = function (event) {
                event = event || window.event;
         ...
         *      adsorb: 吸附距离
         */
        adsorbDrag: function adsorbDrag(obj, adsorb) {
            // 为obj绑定鼠标按下事件
            // 当鼠标在被拖拽元素上按下时，开始拖拽 onmousedown
            obj.onmousedown = function (event) {
                event = event || window.event;
         ...
         *      
         */
        collideDrag: function collideDrag(obj, adsorb, obj1, fun) {
            // 为obj绑定鼠标按下事件
            // 当鼠标在被拖拽元素上按下时，开始拖拽 onmousedown
            obj.onmousedown = function (event) {
                event = event || window.event;
         ...
         *      fun : 碰撞后要设置的事件函数
         */
        collideDrags: function collideDrags(obj, obj1, fun) {
            // 为obj绑定鼠标按下事件
            // 当鼠标在被拖拽元素上按下时，开始拖拽 onmousedown
            obj.onmousedown = function (event) {
                event = event || window.event;
         ...
         *      
         */
        multipleDrag: function multipleDrag(obj, flag, value, obj1, fun) {
            // 为obj绑定鼠标按下事件
            // 当鼠标在被拖拽元素上按下时，开始拖拽 onmousedown
            obj.onmousedown = function (event) {
                event = event || window.event;
         ...
         *      
         */
        dragX: function dragX(obj1, flag, obj2, obj3) {
            // 为进度条方块绑定鼠标按下事件
         ...
         *      -flag:是否保留原始图像
         */
        mosaic: function mosaic(src, level, obj, flag) {
            var img = new Image();
            img.src = src;
            img.onload = function () {
         ...
         *  nums：位数不足时要补充的数
         */
        cover: function cover(num, n, nums) {
         ...
         */
        scratchCard: function scratchCard(obj, image) {
         ...
         */
        divMove: function divMove(obj,speeds,setSpeed) {
            //定义变量存储键盘编码(影响移动方向)
            var choice = 0;
            // 定义变量存储移动速度
            var speed = speeds;
